...
# # For impedance csv file
# Z_s = Constant(msh, PETSc.ScalarType(0))
# # Import impedance structure
# I = np.loadtxt("impedance.csv", delimiter=",")
# Re_Z = I[:, 1]
# Im_Z = I[:, 2]
# Z = Re_Z + 1j * Im_Z
...
a = (
    ...
    - k0**2 * inner(u, v) * dx
    + inner(1j * rho_0 * omega / Z_s * u, v) * ds(3)
...
def frequency_loop(nf):
    ...
    if val % 100 == 0:
        with XDMFFile(
            msh.comm,
            str(output_path.joinpath("air_solution_2_" + str(val) + ".xdmf")),
            "w",
        ) as xdmf:
            xdmf.write_mesh(msh)
...
if __name__ == "__main__":
    ...
    np.savetxt(
        str(output_path.joinpath("p_mic_spectrum_2.csv")), stack_array, delimiter=","
    )

    fig = plt.figure()
    plt.plot(f_axis, 20 * np.log10(np.abs(np.array(p_mic) / 2e-5)))
    plt.savefig(
        str(output_path.joinpath("frequency_response_2.png")), dpi=300
