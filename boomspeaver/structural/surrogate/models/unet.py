...
class ResidualBlock(ConditionedBlock):
    ...
    def __init__(
        ...
        self.conv2 = zero_module(
            conv_layer(
                padding_mode, out_channels, out_channels, kernel_size=3, n_dims=n_dims
            )
        )
        ...
        if in_channels != out_channels:
            self.shortcut = conv_layer(
                padding_mode, in_channels, out_channels, kernel_size=1, n_dims=n_dims
            )
        else:
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor):
        ...
        if self.use_scale_shift_norm:
            ...
            h = (
                self.norm2(h) * (1 + scale) + shift
            )  # where we do -1 or +1 doesn't matter
...
class ConditionalResidualBlock(ConditionedBlock):
    ...
    def __init__(
        ...
        self.conv1 = conv_layer(
            padding_mode,
            in_channels + cond_channels_main,
            out_channels,
            kernel_size=3,
            n_dims=n_dims,
        )
    ...
    def __init__(self, padding_mode: str, n_channels: int, n_dims: int = 1):
        ...
        if n_dims == 1:
            ...
            self.conv = nn.ConvTranspose3d(
                n_channels, n_channels, (4, 4, 4), (2, 2, 2), (1, 1, 1)
            )
        else:
...
class Downsample(nn.Module):
    ...
    def __init__(self, padding_mode: str, n_channels: int, n_dims: int = 1):
        ...
        if n_dims == 1:
            self.conv = nn.Conv1d(
                n_channels, n_channels, 3, 2, 1, padding_mode=padding_mode
            )
        elif n_dims == 2:
            self.conv = nn.Conv2d(
                n_channels,
...
class Unet1D(nn.Module):
    ...
    def __init__(
        ...
        if self.param_conditioning is not None:
            if self.param_conditioning.startswith("scalar"):
                num_params = (
                    1
                    if "_" not in self.param_conditioning
                    else int(self.param_conditioning.split("_")[1])
                )
        ...
        if use1x1:
            self.image_proj = conv_layer(
                padding_mode, insize, n_channels, kernel_size=1, n_dims=n_dims
            )
        else:
            # print(padding_mode, insize, n_channels, n_dims)
            self.image_proj = conv_layer(
                padding_mode, insize, n_channels, kernel_size=3, n_dims=n_dims
            )
    ...
    def forward(
        ...
        if time is not None:
            ...
            assert (
                not self.param_use_time
            ), "Cannot pass time=None after using it in a previous forward pass"
        if z is not None:
            if self.param_conditioning.startswith("scalar"):
                if z.ndim == 1:
                    z = z[:, None]
                for i in range(z.shape[-1]):
                    emb = emb + self.pde_emb[i](
                        fourier_embedding(z[..., i], self.hidden_channels)
                    )
            else:
                raise NotImplementedError(
                    f"Param conditioning {self.param_conditioning} not implemented"
            ...
            assert (
                not self.param_use_cond
            ), "Cannot pass z=None after using it in a previous forward pass"
