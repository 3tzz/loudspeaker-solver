...
class MicrophonePressure:
    ...
    def compute_local_microphones(
        ...
        Returns:
            Two lists (local_cells, local_points) containing
            the local cell indices and the local points.
        """
    ...
    def listen(
        self, u, recompute_collisions: bool = False
    ) -> npt.NDArray[np.complexfloating]:
        """Listen / grasp data from simulation."""
        if recompute_collisions:
            self._local_cells, self._local_position = self.compute_local_microphones()
        if len(self._local_cells) > 0:
            return u.eval(self._local_position, self._local_cells)
        else:
